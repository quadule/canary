---
name: dailies-session-interactive
...
# Dailies session (interactive)

Same recorded QA session as **dailies-session** — explore a flow step by step against one
...
- **Autonomous by default.** Drive the flow yourself, step by step, exactly as in dailies-session.
...
For a hands-off run that just produces a report, use **dailies-session** (the subagent). For a
one-off scrape with no recording, use **dailies-automate**.
...
1. **Start the takeover (you run this):** `npx dailies-cli session takeover "$id" --step
...
3. **The moment they say to continue, run the stop yourself:** `npx dailies-cli session takeover
...
2. Start: `id=$(npx dailies-cli session start --name "<flow>")`. The browser is headed by
...
5. **ACT** — run that action as an intent-named step (`npx dailies-cli run --session "$id" --step
...
- **File uploads:** write the file into the sandbox temp dir first (`writeFile(name, data)`), then
  ...
  `page.setInputFiles` is the Dailies helper that reads the named file host-side from the sandbox
