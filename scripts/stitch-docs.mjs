...
//        cli=npx-cli   substitute `{{cli}}` with `npx dailies-cli`
//                      (default `dailies` — what the CLI help shows)
...
//      snippet (`{{cli}}` pre-substituted with `dailies`), imported by
...
// Markdown surfaces scanned for marker regions. plugins/dailies/skills is a
...
const CLI_SUBSTITUTIONS = {
  dailies: "dailies",
  "npx-cli": "npx dailies-cli",
};

const OPEN_MARKER =
  /^<!-- dailies:snippet ([a-z0-9-]+)((?: [a-z]+=[a-z0-9-]+)*) -->$/;
const CLOSE_MARKER = /^<!-- dailies:end ([a-z0-9-]+) -->$/;
...
function generateTs(snippets) {
  ...
  const lines = [
    ...
    "// `{{cli}}` tokens are pre-substituted with the bare `dailies` invocation.",
    "",
  ];
...
for (const target of await markdownTargets()) {
  ...
  if (!source.includes("<!-- dailies:snippet ")) {
    continue;
  }
  await reconcile(
    target,
