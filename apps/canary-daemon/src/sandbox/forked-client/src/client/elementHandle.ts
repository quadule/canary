...
export class ElementHandle<T extends Node = Node> extends JSHandle<T> implements api.ElementHandle {
  ...
  async screenshot(
    ...
    const copy: channels.ElementHandleScreenshotParams = {
      ...options,
      path: undefined,
      mask: undefined,
      timeout: this._frame._timeout(options),
  ...
  waitForSelector(
    selector: string,
    options?: channels.ElementHandleWaitForSelectorOptions & TimeoutOptions
...
type SetInputFilesFiles = Pick<
  channels.ElementHandleSetInputFilesParams,
  "payloads" | "localPaths" | "localDirectory" | "streams" | "directoryStream"
>;

function filePayloadExceedsSizeLimit(payloads: FilePayload[]) {
  return (
    payloads.reduce((size, item) => size + (item.buffer ? item.buffer.byteLength : 0), 0) >=
    fileUploadSizeLimit
  );
}
    ...
    if (context._connection.isRemote()) {
      const files = localDirectory
        ? (
            await platform
              .fs()
              .promises.readdir(localDirectory, { withFileTypes: true, recursive: true })
          )
            .filter((f) => f.isFile())
            .map((f) => platform.path().join(f.parentPath, f.name))
        : localPaths!;
      const { writableStreams, rootDir } = await context._wrapApiCall(
        async () =>
          context._channel.createTempFiles({
            ...
            items: await Promise.all(
              files.map(async (file) => {
                ...
                return {
                  name: localDirectory
                    ? platform.path().relative(localDirectory, file)
                    : platform.path().basename(file),
                  lastModifiedMs,
                };
              })
            ),
