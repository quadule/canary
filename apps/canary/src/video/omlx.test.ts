...
describe("omlxBaseUrl", () => {
  it("defaults to localhost and honors the override", () => {
    ...
    expect(omlxBaseUrl({ CANARY_OMLX_URL: "http://host:9000" })).toBe(
      "http://host:9000"
    );
  });
});
...
describe("describeSpeechCurl", () => {
  it("is copy-pasteable and NEVER contains the real key", () => {
    ...
    expect(curl).toContain("$CANARY_OMLX_API_KEY");
...
describe.skipIf(!reachable)("oMLX live TTS", () => {
  const out = path.join(os.tmpdir(), `canary-omlx-test-${process.pid}.wav`);
  ...
  it("synthesizes a real WAV via the provider", async () => {
    ...
    await tts?.synthesize("Canary checks the login flow.", out);
