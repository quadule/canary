...
//
// Enabled by setting $CANARY_IMAGE_URL (e.g. http://127.0.0.1:8080). Optional
// $CANARY_IMAGE_MODEL picks the model; $CANARY_IMAGE_API_KEY adds a Bearer header.
//
...
export function describeImageCurl(args: {
  ...
  const auth = args.hasKey
    ? ['-H "Authorization: Bearer $CANARY_IMAGE_API_KEY"']
    : [];
  return [
    "curl -s -X POST",
...
// Resolve the local-image provider when $CANARY_IMAGE_URL is set. Returns just
...
export function resolveLocalImage(opts: {
  ...
  const baseUrl = env.CANARY_IMAGE_URL?.trim();
  if (!baseUrl) {
    return { enabled: false };
  }
  const config: LocalImageConfig = {
    baseUrl: baseUrl.replace(/\/+$/, ""),
    apiKey: env.CANARY_IMAGE_API_KEY?.trim() || undefined,
    model: env.CANARY_IMAGE_MODEL?.trim() || undefined,
  };
