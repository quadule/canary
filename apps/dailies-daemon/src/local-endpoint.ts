...
export function getDailiesBaseDir(homedir = os.homedir()): string {
  return path.join(homedir, ".dailies");
}
...
export function getDaemonEndpoint(
  ...
  if (platform === "win32") {
    ...
    return `\\\\.\\pipe\\dailies-daemon-${username}`;
  }

  return path.join(getDailiesBaseDir(homedir), "daemon.sock");
}

export function getPidPath(homedir = os.homedir()): string {
  return path.join(getDailiesBaseDir(homedir), "daemon.pid");
}

export function getBrowsersDir(homedir = os.homedir()): string {
  return path.join(getDailiesBaseDir(homedir), "browsers");
}

export function getSessionsDir(homedir = os.homedir()): string {
  return path.join(getDailiesBaseDir(homedir), "sessions");
}
