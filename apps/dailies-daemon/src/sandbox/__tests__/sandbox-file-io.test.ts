...
describe.sequential("QuickJS sandbox file I/O", () => {
  ...
  beforeAll(async () => {
    ...
    browserRootDir = await mkdtemp(
      path.join(os.tmpdir(), "dailies-quickjs-file-io-")
    );
  ...
  it("saves screenshot buffers into the controlled temp directory", async () => {
    ...
    const expectedPath = await resolveDailiesTempPath(requestedName);
    ...
    expect(
      result.savedPath.startsWith(`${path.resolve(DAILIES_TMP_DIR)}${path.sep}`)
    ).toBe(true);
  ...
  it("rejects symlinked temp-file targets", async () => {
    ...
    const symlinkPath = path.join(DAILIES_TMP_DIR, symlinkName);
