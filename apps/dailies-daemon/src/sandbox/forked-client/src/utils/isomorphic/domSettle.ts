...
// or after ceilingMs regardless. Dailies's own cursor/ripple/caption/vignette
...
export const buildDomQuiescenceJs = (
  ...
  new Promise((resolve) => {
    const isOverlay = (node) => {
      ...
      while (el) {
        const t = el.tagName;
        if (t === "DAILIES-VIRTUAL-CURSOR" || t === "DAILIES-CLICK-RIPPLE" || t === "DAILIES-CAPTION" || t === "DAILIES-VIGNETTE") {
          return true;
        }
        el = el.parentElement;
      }
