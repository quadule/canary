...
import { ensureDailiesTempDir } from "./temp-files.js";

const BASE_DIR = getDailiesBaseDir();
  ...
  if (targetSession) {
    ...
    if (!sessions.has(targetSession)) {
      await writeMessage(socket, {
        id: request.id,
        type: "error",
        message: `No active session "${targetSession}". Start one with \`dailies session start\`.`,
      });
      return false;
    }
...
async function handleExecute(
  ...
  if (targetSession && sessions.isRecording(targetSession)) {
    await writeMessage(socket, {
      id: request.id,
      type: "error",
      message: `Session "${targetSession}" is in an interactive takeover; stop it with \`dailies session takeover ${targetSession} --stop\` before running steps.`,
    });
    return;
  }
    ...
    case "browser-stop":
      ...
      if (sessions.isSessionBrowser(request.browser)) {
        await writeMessage(socket, {
          id: request.id,
          type: "error",
          message: `"${request.browser}" is a session browser; tear it down with \`dailies session end\` or \`dailies session abort\`.`,
        });
        return;
      }
