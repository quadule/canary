#!/usr/bin/env node
// The Dailies session viewer, runnable straight from npm — the trace-viewer
// pattern, for dailies:
//
//   npx dailies-ui                    # browse ~/.dailies/sessions
//   npx dailies-ui --dir ./artifacts  # browse a specific folder
//   npx dailies-ui --no-open          # don't auto-open a browser
//
...
if (!existsSync(serverEntry)) {
  process.stderr.write(
    "dailies viewer: bundled server (dist/server/entry.mjs) not found — this dailies-ui install looks incomplete.\n"
  );
  process.exit(1);
}
...
const root = path.resolve(
  arg("--dir", path.join(homedir(), ".dailies", "sessions"))
);
...
if (!ready) {
  child.kill("SIGTERM");
  process.stderr.write(
    "dailies viewer: server did not become ready in time.\n"
  );
  process.exit(1);
}

process.stdout.write(
  `dailies viewer on ${url}\n  source: ${root}\n  Press Ctrl-C to stop.\n`
);
