...
describe("roots registry (DAILIES_DIR-isolated)", () => {
  let home: string;
  beforeEach(async () => {
    home = await mkdtemp(path.join(os.tmpdir(), "dailies-home-"));
    process.env.DAILIES_DIR = home;
  });
  afterEach(async () => {
    Reflect.deleteProperty(process.env, "DAILIES_DIR");
    Reflect.deleteProperty(process.env, "DAILIES_UI_ROOT");
  ...
  it("selects DAILIES_UI_ROOT (the --dir launch root) by default", async () => {
    process.env.DAILIES_UI_ROOT = "/tmp/launch-root";
    const { lastRootId, roots } = await loadRoots();
  ...
  it("roots a --dir that points at a single session dir to its parent folder", async () => {
    ...
    process.env.DAILIES_UI_ROOT = sessionDir;
    const { lastRootId, roots } = await loadRoots();
