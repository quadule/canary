...
// A launch root (DAILIES_UI_ROOT, set from `--dir`) is meant to be a folder
...
// flow opens `--dir ~/.dailies/sessions/<id>`: without this, that session dir
...
// Guarantee the default ~/.dailies/sessions root is always registered.
...
// Load the registry, seeding the default root and (if `dailies ui --dir` set
// DAILIES_UI_ROOT) the launch root, which becomes the selected one. Persists
...
export function loadRoots(): Promise<{
  ...
  return serializeConfig(async () => {
    ...
    const envRoot = process.env.DAILIES_UI_ROOT?.trim();
    if (envRoot) {
