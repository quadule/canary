...
describe("acestepBaseUrl", () => {
  it("defaults to :8001 and honors the override", () => {
    ...
    expect(acestepBaseUrl({ DAILIES_ACESTEP_URL: "http://h:9" })).toBe(
      "http://h:9"
    );
  });
});
    ...
    expect(
      (
        buildMusicPayload({
          ...
          seconds: 0,
          instrumental: true,
        }).audio_config as { duration: number }
      ).duration
    ).toBe(1);
    expect(
      buildMusicPayload({
...
describe("audioFromResponse", () => {
  ...
  it("returns null when there's no audio", () => {
    expect(
      audioFromResponse({ choices: [{ message: { content: "hi" } }] })
    ).toBeNull();
...
// (heavy + slow), gated behind DAILIES_TEST_ACESTEP=1 so it never runs in CI.
describe.skipIf(process.env.DAILIES_TEST_ACESTEP !== "1")(
  "ACE-Step live",
  () => {
    it("generates an instrumental bed via the provider", async () => {
      ...
      const out = path.join(os.tmpdir(), `dailies-acestep-${process.pid}.wav`);
      try {
...
describe("describeMusicCurl", () => {
  it("redacts the key and includes auth only when present", () => {
    ...
    expect(withKey).toContain("$DAILIES_ACESTEP_API_KEY");
