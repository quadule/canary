...
// Opt-in (DAILIES_TEST_ARCHIVE=1) so it never flakes CI on network.
describe.skipIf(process.env.DAILIES_TEST_ARCHIVE !== "1")(
  "archive.org live",
  () => {
    ...
    it("provider fetches, trims, and writes a bed (needs ffmpeg)", async () => {
      ...
      const out = path.join(os.tmpdir(), `dailies-archive-${process.pid}.wav`);
      try {
