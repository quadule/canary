...
// How `dailies ui` should launch the dailies-ui web app:
...
// Walk up from this module's location looking for the apps/dailies-ui workspace.
// Works whether the CLI runs bundled (apps/dailies/dist/cli.js) or via tsx
// (apps/dailies/src/cli.ts) — both sit under the monorepo root.
function findWorkspaceUiDir(): string | null {
  let dir = path.dirname(fileURLToPath(import.meta.url));
  let parent = path.dirname(dir);
  while (dir !== parent) {
    const ui = path.join(dir, "apps", "dailies-ui");
...
export async function resolveUiServer(): Promise<ResolvedUiServer | null> {
  ...
  const override = process.env.DAILIES_UI_SERVER;
