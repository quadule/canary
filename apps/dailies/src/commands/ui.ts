...
import { sessionsRootDir } from "dailies-daemon-client";
...
// Launch the dailies-ui web app in the FOREGROUND (unlike the detached daemon):
...
export async function uiCommand(args: UiArgs): Promise<number> {
  ...
  if (!resolved) {
    process.stderr.write(
      "dailies ui: could not locate the dailies-ui app. Build it with `pnpm --filter dailies-ui build`, or set DAILIES_UI_SERVER.\n"
    );
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write("dailies ui: could not allocate a port.\n");
    return 1;
  }

  const url = `http://${host}:${port}`;
  const baseEnv = { ...process.env, DAILIES_UI_ROOT: root };
  ...
  if (resolved.kind === "standalone") {
    ...
    process.stderr.write(
      "dailies ui: no production build found; starting the dev server (slower first paint). Run `pnpm --filter dailies-ui build` for fast startup.\n"
    );
    child = spawn(
      process.execPath,
  ...
  if (!ready) {
    child.kill("SIGTERM");
    await exited;
    process.stderr.write("dailies ui: server did not become ready in time.\n");
    return 1;
  }
