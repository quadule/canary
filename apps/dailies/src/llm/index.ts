  ...
  for (const provider of providers) {
    ...
    if ("value" in outcome) {
      if (declined.length > 0) {
        rest.log.info(
          { declined, provider: provider.id },
          `${rest.label}: generated with ${provider.describe()} after ${declined.length} provider(s) declined`
        );
      }
      return { provider: provider.id, value: outcome.value };
    }
