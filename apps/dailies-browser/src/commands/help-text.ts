// Help prose for the `dailies-browser` engine CLI. The sandbox rules, script
// API, and LLM usage guide come from dailies-cli-kit — the single source of
// truth shared with the `dailies` orchestrator — so the two CLIs' help cannot
...
// Engine-specific: how to invoke dailies-browser (heredoc, run FILE, --browser,
...
const INVOCATION_STYLES = `Primary invocation styles:
  ...
  dailies-browser --browser my-project < script.js
...
export const CLI_LONG_ABOUT = `Dailies Browser is a CLI for controlling local or external browsers with JavaScript scripts.
...
export const RUN_LONG_ABOUT =
  ...
  "Use top-level flags before `run`, for example `dailies-browser --browser my-project run script.js`.";
